...
class PingService {
  ...
  writeService(service) {
    ...
    // console.log("Writing service to DB:", flattenedService);
    pingStatusDB.data[serviceId] = flattenedService;
    pingStatusDB.write();
  ...
  updateStatus(service) {
    ...
    if (oldStatus !== service.status) {
      this.updateEquipmentServiceStatus(service);
    }
  ...
  updateEquipmentServiceStatus(service) {
    console.log("Updating equipment service status:", service);
    const serviceID = service.serviceId;
    const equipmentGroup = equipmentDB.data.equipmentGroups.find((group) =>
    ...
    if (equipmentGroup) {
      ...
      if (serviceIndex !== -1) {
        ...
        equipmentDB.write();
        getEquipmentGroupsNamespace()?.emit(
          "equipmentGroups",
  ...
  getStatus(serviceId) {
    return pingStatusDB.data[serviceId];
  }
